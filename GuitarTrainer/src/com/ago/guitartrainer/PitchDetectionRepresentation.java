...
public class PitchDetectionRepresentation {
    public PitchDetectionRepresentation(double pitch_, int string_no_, int fret_) {
        pitch = pitch_;
        string_no = string_no_;
        fret = fret_;
        creation_date_ = System.currentTimeMillis();
        string_detected = true;
    }

    public PitchDetectionRepresentation(double pitch_) {
        pitch = pitch_;
        creation_date_ = System.currentTimeMillis();
    ...
    public int getAlpha() {
        ...
        if (age > LIFE_TIME_MS)
            return 0;
        if (age < BRIGHT_TIME_MS)
            return 255;
        return (int) Math.floor(255 - (age - BRIGHT_TIME_MS) * 1.0 / (LIFE_TIME_MS - BRIGHT_TIME_MS) * 255);
    }
